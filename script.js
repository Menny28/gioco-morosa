...
const ACHIEVEMENTS = [
  ...
  { id: "cento-giorni", icon: "📅", title: "100 Giorni Insieme", desc: "100 giorni passati insieme a Tommy",
    check: s => Math.floor((Date.now() - START_DATE.getTime()) / 86400000) >= 100 },
];
      ...
      <div class="memory-grid">
        ${memoryState.cards.map((c, i) => `
          <button class="memory-card ${c.matched ? "matched" : ""}" data-idx="${i}" ${c.matched || memoryState.busy ? "disabled" : ""}>
            ${c.matched || memoryState.flipped.includes(i) ? c.symbol : ""}
          </button>
        `).join("")}
      </div>
    `;
...
function renderCatchView() {
  ...
  if (!catchState) {
    inner = `
      <div class="sheet-note">Tocca più cuori possibile in ${CATCH_DURATION_SEC} secondi! Partite rimaste: ${roundsLeft}/${catchLimiter.max}</div>
...
function startCatchGame() {
  ...
  save();
  updateBars();
  catchState = { score: 0, timeLeft: CATCH_DURATION_SEC, spawnTimer: null, countdownTimer: null };
  renderCatchView();
  const area = document.getElementById("catch-area");
  catchState.spawnTimer = setInterval(() => spawnHeart(area), 650);
...
function stopCatchGame(discard) {
  if (catchState) {
    clearInterval(catchState.spawnTimer);
...
function finishCatchGame() {
  ...
  let affettoGain, coinGain, msg;
  if (score >= 8) { affettoGain = 4; coinGain = 2; msg = "Riflessi fulminei! Che squadra 💗"; }
  else if (score >= 4) { affettoGain = 2; coinGain = 1; msg = "Bel punteggio!"; }
