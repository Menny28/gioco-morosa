...
// Oggetti acquistabili al negozio.
const SHOP_ITEMS = [
  { id: "pizza", label: "🍕 Pizza", cost: 3, desc: "Riempie il frigo di una porzione" },
  { id: "snack", label: "🐟 Snack per i gatti", cost: 2, desc: "Ai gatti piace tantissimo (+affetto)" },
];
...
function mangia() {
  if (state.fridge <= 0 || isBusy) return;
  state.fridge -= 1;
  state.fame = clamp(state.fame + 30);
  addAffetto(2);
...
function compra(itemId) {
  ...
  if (item.id === "pizza") state.fridge += 1;
  if (item.id === "snack") addAffetto(1);
  save();
  updateBars();
  renderNegozioSheet();
}
...
function renderFridgeSheet() {
  el.sheetContent.innerHTML = `
    <h3>🧊 Frigo</h3>
    <div class="sheet-row">
      <span>🍕 Pizza disponibili: ${state.fridge}</span>
      <button id="mangia-btn" ${state.fridge <= 0 ? "disabled" : ""}>Mangia</button>
    </div>
    <div class="sheet-note">${state.fridge <= 0 ? "Il frigo è vuoto — passa dal negozio! 🛒" : "Un morso e la fame passa."}</div>
  `;
  document.getElementById("mangia-btn")?.addEventListener("click", mangia);
}
...
function renderGiocoSheet() {
  el.sheetContent.innerHTML = `
    ...
    <div class="rps-row">
      <button data-rps="0">🪨</button>
      <button data-rps="1">📄</button>
      <button data-rps="2">✂️</button>
    </div>
...
function giocaRPS(choice) {
  ...
  if (outcome === 1) {
    state.coins += 2;
    state.energia = clamp(state.energia - 3);
    addAffetto(3);
    resultEl.textContent = `Tu: ${RPS[choice]} — Tommy: ${RPS[cpu]} → Hai vinto! +2 💰`;
    setDiary(randomOf(MESSAGES.playWin));
    setTimeout(() => showTransient("felice", 1200), 950);
  } else if (outcome === -1) {
    state.energia = clamp(state.energia - 3);
    resultEl.textContent = `Tu: ${RPS[choice]} — Tommy: ${RPS[cpu]} → Ha vinto Tommy!`;
    setDiary(randomOf(MESSAGES.playLose));
  } else {
    resultEl.textContent = `Tu: ${RPS[choice]} — Tommy: ${RPS[cpu]} → Pareggio!`;
...
tick();
updateBars();
setInterval(tick, 15000); // ricalcola ogni 15s mentre la pagina è aperta
