...
// Countdown segreto per l'anniversario: si ripete ogni anno, e per tutto
// il giorno dell'anniversario stesso (00:00-23:59) resta nascosto invece
// di mostrare un conto alla rovescia a zero.
// MODIFICA QUI se la data dovesse mai cambiare.
const ANNIVERSARY_MONTH = 6;  // giugno (1-indicizzato, come in SPECIAL_DATES)
const ANNIVERSARY_DAY = 22;

function getNextAnniversaryTarget() {
  const now = new Date();
  const isAnniversaryToday = now.getMonth() + 1 === ANNIVERSARY_MONTH && now.getDate() === ANNIVERSARY_DAY;
  if (isAnniversaryToday) return null;
  const year = now.getFullYear();
  let target = new Date(year, ANNIVERSARY_MONTH - 1, ANNIVERSARY_DAY, 0, 0, 0, 0);
  if (now.getTime() >= target.getTime()) {
    target = new Date(year + 1, ANNIVERSARY_MONTH - 1, ANNIVERSARY_DAY, 0, 0, 0, 0);
  }
  return target;
}
...
function openAnniversaryCountdown() {
  const target = getNextAnniversaryTarget();
  if (!target) {
    openNote("Oggi è il giorno! Buon anniversario 💛", "🎉 È oggi!");
    return;
  }
  const tick = () => {
    el.noteText.textContent = `Manca ancora:\n${formatCountdown(target.getTime() - Date.now())}`;
  };
  el.noteTitle.textContent = "💛 Al nostro anniversario";
  el.noteTitle.style.display = "block";
  tick();
...
// Quiz "Quanto mi conosci": una sola domanda al giorno, pescata a caso da
// questo mazzo senza ripetizioni finché non le hai viste tutte. "correct"
// è l'indice (0-based) della risposta giusta dentro "options".
// MODIFICA QUI per aggiungere, togliere o cambiare le domande.
const QUIZ_ITEMS = [
  { id: "q1", question: "In quale luogo esatto ci siamo dati il nostro primo bacio?",
    options: ["A casa di Tommy", "Al parco", "In macchina"], correct: 0 },
  { id: "q2", question: "Cosa ho ordinato al nostro primo appuntamento?",
    options: ["Gelato al cioccolato", "Gelato alla nocciola", "Gelato alla fragola"], correct: 1 },
  { id: "q3", question: "Chi dei due ci mette più tempo a prepararsi per uscire?",
    options: ["Tommy", "Benny"], correct: 1 },
  { id: "q4", question: "Chi è il più bravo a cucinare?",
    options: ["Benny", "Tommy"], correct: 1 },
  { id: "q5", question: "Qual è il mio colore preferito?",
    options: ["Blu", "Nero", "Rosso"], correct: 2 },
  { id: "q6", question: "Da quale lato del letto dormo di solito?",
    options: ["Sinistro", "Destro", "Cambia sempre lato"], correct: 0 },
  { id: "q7", question: "Dove ci siamo visti la prima volta?",
    options: ["A una festa", "In pizzeria", "In palestra"], correct: 1 },
  { id: "q8", question: "Quando è il nostro anniversario?",
    options: ["12 giugno", "22 luglio", "22 giugno"], correct: 2 },
  { id: "q9", question: "Che giorno della settimana è stata la nostra prima uscita?",
    options: ["Sabato", "Giovedì", "Martedì"], correct: 1 },
  { id: "q10", question: "Preferisco dolce o salato?",
    options: ["Dolce", "Salato"], correct: 1 },
  { id: "q11", question: "Chi è più bello?",
    options: ["Benny", "Tommy"], correct: 1 },
  { id: "q12", question: "Qual è il mio cibo preferito?",
    options: ["Pizza", "Hamburger", "Sushi"], correct: 2 },
  { id: "q13", question: "Cosa siamo andati a mangiare alla nostra prima cena fuori?",
    options: ["Pesce", "Carne", "Pizza"], correct: 1 },
  { id: "q14", question: "Quanto sono alto?",
    options: ["1,75 m", "1,90 m", "1,83 m"], correct: 2 },
  { id: "q15", question: "Qual è la mia bibita preferita?",
    options: ["Coca-Cola", "Red Bull", "Fanta"], correct: 1 },
  { id: "q16", question: "Chi ha detto per primo \"ti amo\"?",
    options: ["Tommy", "Benny", "Insieme, nello stesso momento"], correct: 2 },
  { id: "q17", question: "Quante macchine ha avuto Tommy in 3 anni di patente?",
    options: ["1", "3", "5"], correct: 1 },
  { id: "q18", question: "Di che colore ho gli occhi?",
    options: ["Marroni", "Azzurri", "Verdi"], correct: 2 },
  { id: "q19", question: "Che numero di scarpe porto?",
    options: ["43", "44,5", "45"], correct: 1 },
  { id: "q20", question: "Qual è stata la nostra prima vacanza insieme?",
    options: ["Roma", "Barcellona", "Parigi"], correct: 2 },
  { id: "q21", question: "Cosa ci siamo regalati ai nostri primi compleanni insieme?",
    options: ["Un braccialetto", "Un album fotografico", "Un peluche"], correct: 1 },
  { id: "q22", question: "Mi ami?",
    options: ["No", "Boh", "Sì, moltissimo"], correct: 2 },
  { id: "q23", question: "Di che colore è Pino?",
    options: ["Arancione", "Grigio", "Nero"], correct: 1 },
  { id: "q24", question: "Quanti cucchiaini di zucchero metto nel caffè?",
    options: ["1", "1 e ¾ (1,75)", "2"], correct: 1 },
];
const QUIZ_CORRECT_AFFETTO = 2;
const QUIZ_CORRECT_COIN = 1;
...
function loadState() {
  ...
  if (!raw) {
    raw = {
      ...
      quizOrder: [], quizOrderPos: 0, quizLastDate: "",
      quizAnsweredToday: false, quizAnsweredCorrectToday: null, totalQuizCorrect: 0,
      lastUpdate: Date.now(),
    };
  }
  ...
  if (!Array.isArray(raw.quizOrder)) raw.quizOrder = [];
  if (typeof raw.quizOrderPos !== "number") raw.quizOrderPos = 0;
  if (typeof raw.quizLastDate !== "string") raw.quizLastDate = "";
  if (typeof raw.quizAnsweredToday !== "boolean") raw.quizAnsweredToday = false;
  if (typeof raw.quizAnsweredCorrectToday === "undefined") raw.quizAnsweredCorrectToday = null;
  if (typeof raw.totalQuizCorrect !== "number") raw.totalQuizCorrect = 0;
...
el.noteClose.addEventListener("click", () => {
  el.noteBackdrop.classList.remove("show");
  if (countdownIntervalId) { clearInterval(countdownIntervalId); countdownIntervalId = null; }
});
el.secretHeart.addEventListener("click", openAnniversaryCountdown);
...
const ACHIEVEMENTS = [
  ...
  { id: "quiz-cervellone", icon: "🧠", title: "Cervellone", desc: "5 risposte giuste al quiz del giorno",
    check: s => s.totalQuizCorrect >= 5 },
];
...
// Fisher-Yates: un nuovo ordine casuale degli indici del mazzo di domande.
...
// Si occupa di avere sempre pronta "la domanda di oggi": ne pesca una nuova
// solo quando cambia il giorno, senza ripetizioni finché il mazzo non è
// esaurito, poi rimescola per un nuovo giro.
function ensureTodaysQuiz() {
  const key = todayKey();
  if (state.quizLastDate === key) return;
  const firstTime = !state.quizLastDate;
  if (!firstTime) state.quizOrderPos += 1;
  if (state.quizOrder.length === 0 || state.quizOrderPos >= state.quizOrder.length) {
    state.quizOrder = shuffledIndices(QUIZ_ITEMS.length);
    state.quizOrderPos = 0;
  }
  state.quizLastDate = key;
  state.quizAnsweredToday = false;
  state.quizAnsweredCorrectToday = null;
  save();
}

function renderQuizSheet() {
  ensureTodaysQuiz();
  const q = QUIZ_ITEMS[state.quizOrder[state.quizOrderPos]];
  let inner;
  if (state.quizAnsweredToday) {
    const resultMsg = state.quizAnsweredCorrectToday
      ? "Esatto! Mi conosci benissimo 💛"
      : `Non proprio! La risposta giusta era: ${q.options[q.correct]}`;
    inner = `
      <div class="quiz-question">${q.question}</div>
      <div class="quiz-result">${resultMsg}</div>
      <div class="sheet-note">Torna domani per una nuova domanda ✨</div>
    `;
  } else {
    inner = `
      <div class="quiz-question">${q.question}</div>
      <div class="quiz-options">
        ${q.options.map((opt, i) => `<button class="quiz-option-btn" data-opt="${i}">${opt}</button>`).join("")}
      </div>
    `;
  }
  el.sheetContent.innerHTML = `<h3>🧠 Quanto mi conosci</h3>${inner}`;
  el.sheetContent.querySelectorAll("[data-opt]").forEach(btn => {
    btn.addEventListener("click", () => answerQuiz(Number(btn.dataset.opt), q));
  });
}

function answerQuiz(selected, q) {
  if (state.quizAnsweredToday) return;
  state.quizAnsweredToday = true;
  const correct = selected === q.correct;
  state.quizAnsweredCorrectToday = correct;
  if (correct) {
    state.totalQuizCorrect += 1;
    state.coins += QUIZ_CORRECT_COIN;
    addAffetto(QUIZ_CORRECT_AFFETTO);
    spawnParticles(["🧠", "💛", "✨"], 6);
  }
  save();
  updateBars();
...
document.querySelectorAll(".action-btn").forEach(btn => {
  btn.addEventListener("click", () => {
    ...
    else if (action === "quiz") { renderQuizSheet(); openSheet(); }
  });
});

/* ---------- SUONI LEGGERI SUI TASTI ---------- */

// Un piccolo "tock" morbido, generato al volo (nessun file audio da
// scaricare), che parte ad ogni tocco di un pulsante — in stile iOS,
